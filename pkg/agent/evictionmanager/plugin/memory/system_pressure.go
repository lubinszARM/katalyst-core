...
func (s *SystemPressureEvictionPlugin) detectOnlineMemPSI() error {
	...
	if pressure.Avg10 >= 15 {
		...
		if avgAvg10 >= 5 {
			...
			general.Infof("Detect high mem.pressure: psi=%v, time=%v, dur=%v.\n", avgAvg10, s.onlineMemHighPressureHitThresAt, int64(diff))
			if int64(diff) >= 20 {
				s.isUnderSystemPressure = true
				s.systemAction = actionReclaimedEviction
				general.Infof("Trigger actionReclaimedEviction, thresholdMet: psi=%+v, duration=%+v", avgAvg10, int64(diff))
			}
		}
	} else {
		//general.Infof("BBLU-SOME kubepods/burstable  mem.psi: psi=%+v", pressure.Avg10)
		s.onlineMemHighPressureHitThresAt = time.Time{}
	}
	return nil
}
