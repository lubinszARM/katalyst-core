...
func setHostTCPMem(memTotal uint64) error {
	...
	fmt.Printf("BBLU555 ratio=%v, newLimit=%d, oldLimit=%d\n", tcpMemRatio, newUpperLimit, tcpMem[2])
	if (newUpperLimit != tcpMem[2]) && (newUpperLimit > tcpMem[1]) {
		...
		fmt.Printf("BBLU666 ratio=%v, newLimit=%d, oldLimit=%d\n", tcpMemRatio, newUpperLimit, tcpMem[2])
...
func setCg1TCPMem(podUID, containerID string, memLimit, memTCPLimit int64) error {
	...
	fmt.Printf("BBLU777 cg=%v, ratio:%d, %d, %d, %d...\n", cgroupPath, int64(sockMemConfig.cgroupTCPMemRatio), newMemTCPLimit, memLimit, memTCPLimit)
	if newMemTCPLimit != memTCPLimit {
		...
		fmt.Printf("BBLU888 TCPMemLimitInBytes: %v, old value=%d, new value=%d\n", cgroupPath, memTCPLimit, newMemTCPLimit)
