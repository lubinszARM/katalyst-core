...
func setHostFragBoostFactor(filePath string, threshold int) error {
	_, err := os.Stat(filePath)
	if err != nil {
		return err
	}

	err = os.WriteFile(filePath, []byte(fmt.Sprintf("%d", uint64(threshold))), 0644)
	if err != nil {
		return fmt.Errorf("failed to write to %s, err %v", filePath, err)
	}
