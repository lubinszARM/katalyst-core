...
func triggerMemoryCompaction(conf *coreconfig.Configuration, metaServer *metaserver.MetaServer, emitter metrics.MetricEmitter) {
	...
	for _, numaID := range metaServer.CPUDetails.NUMANodes().ToSliceNoSortInt() {
		...
		// Step 4, if memory compression is not effective, extend the check interval
...
func SetMemCompact(conf *coreconfig.Configuration,
	...
	triggerMemoryCompaction(conf, metaServer, emitter)
}
