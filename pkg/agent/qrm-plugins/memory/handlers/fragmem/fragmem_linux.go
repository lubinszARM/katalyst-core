...
func memCompacWithBestEffort(conf *coreconfig.Configuration, metaServer *metaserver.MetaServer, emitter metrics.MetricEmitter) {
	...
	for _, numaID := range metaServer.CPUDetails.NUMANodes().ToSliceNoSortInt() {
		// Step 1, get fragScore from Malachite
		...
		/*
		 * If mem frag-score shows no significant improvement after compaction
		 * (frag_score < (fragScoreWatermark-minFragScoreGap)),
		 * it indicates that the system does not have so much physical memory fragmentation.
		 */
...
func asyncMemCompaction(conf *coreconfig.Configuration, metaServer *metaserver.MetaServer, emitter metrics.MetricEmitter) {
	// Run memCompacWithBestEffort in a goroutine
	go func() {
...
func SetMemCompact(conf *coreconfig.Configuration,
	...
	// Step3, async user space memory compaction will be trigger while exceeding fragScoreWatermark(default:80).
	asyncMemCompaction(conf, metaServer, emitter)
}
