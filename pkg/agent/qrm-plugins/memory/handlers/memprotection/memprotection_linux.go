...
func calculateMemSoftLimit(relCgPath string, ratio uint64) (uint64, error) {
	...
	general.Infof("BBLU456 got: target=%v, usr=%v, ratio=%v, cg=%v, limit=%v, usage=%v, file-inactive=%v, ulim=%v..\n", softLimit, userSoftLimit, ratio, relCgPath, memStat.Limit, memStat.Usage, memStat.FileInactive, cgroupMemoryUnlimited)
