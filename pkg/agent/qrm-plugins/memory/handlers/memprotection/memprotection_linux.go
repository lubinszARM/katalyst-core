...
func getUserSpecifiedMemoryProtectionInBytes(memLimit uint64, ratioUser string) uint64 {
	...
	softLimit := uint64(float64(memLimit) / 100.0 * float64(ratio))
...
func applyMemSoftLimitQoSLevelConfig(conf *coreconfig.Configuration,
	...
	for _, pod := range podList {
		...
		for _, containerStatus := range pod.Status.ContainerStatuses {
			...
			general.Infof("BBLU11 got: limit=%x, %x, %x, ratio=%v.\n", memStat.Limit, memStat.Usage, memFileInactive, qosLevelDefaultValue)

			// Step2, Reserve a certain ratio of file memory for high-QoS cgroups.
			userSoftLimit := getUserSpecifiedMemoryProtectionInBytes(memStat.Limit, qosLevelDefaultValue)
			if userSoftLimit == 0 {
			...
			softLimit := calculatedBestSoftLimit(memStat.Usage, uint64(memFileInactive), userSoftLimit)
