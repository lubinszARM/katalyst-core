...
func calculatedBestSoftLimit(memUsage, memFileInactive, userSoftLimit float64) int64 {
	if memFileInactive > memUsage {
		return 0
	}
	minSoftLimit := memUsage - memFileInactive + cgroupMemory128M
	maxSoftLimit := memUsage
	softLimit := int64(general.Clamp(userSoftLimit, minSoftLimit, maxSoftLimit))
	return softLimit
}

func getUserSpecifiedMemoryProtectionInBytes(memLimit float64, ratioUser string) int64 {
	...
	softLimit := uint64(memLimit / 100.0 * float64(ratio))
	softLimit = general.AlignToPageSize(softLimit)
	return int64(softLimit)
}
...
func applyMemSoftLimitQoSLevelConfig(conf *coreconfig.Configuration,
	...
	for _, pod := range podList {
		...
		for _, containerStatus := range pod.Status.ContainerStatuses {
			...
			general.Infof("BBLU11 got: limit=%x, %x, %x, ratio=%v.\n", memLimit, memUsage, memFileInactive, qosLevelDefaultValue)

			// Step2, Reserve a certain ratio of file memory for high-QoS cgroups.
			userSoftLimit := getUserSpecifiedMemoryProtectionInBytes(memLimit, qosLevelDefaultValue)
			if userSoftLimit == 0 {
			...
			softLimit := calculatedBestSoftLimit(memUsage, memFileInactive, float64(userSoftLimit))
			general.Infof("BBLU22 got softlimit %v, %v.\n", userSoftLimit, softLimit)
