...
func calculatedBestSoftLimit(memUsage, memFileInactive, userSoftLimit uint64) uint64 {
	...
	fmt.Printf("BBLU-calcu:%v, %v, %v, %v..\n", memUsage, memFileInactive, userSoftLimit, softLimit)
	return softLimit
}
...
func applyMemSoftLimitCgroupLevelConfig(conf *coreconfig.Configuration,
	...
	// applyMemSoftLimitCgroupLevelConfig does not support cg1 env for now.
	if !common.CheckCgroup2UnifiedMode() {
		general.Infof("skip applyMemSoftLimitCgroupLevelConfig in cg1 env")
		return
	}
...
func applyMemSoftLimitQoSLevelConfig(conf *coreconfig.Configuration,
	...
	fmt.Printf("BBLU applyMemSoftLimitQoSLevelConfig ...\n")
	...
	for _, pod := range podList {
		...
		for _, containerStatus := range pod.Status.ContainerStatuses {
			...
			fmt.Printf("BBLU222 set ok:cg=%v.ratio=%v,limit=%v..\n", relCgPath, ratio, softLimit)
			_ = emitter.StoreInt64(metricNameMemLow, int64(softLimit), metrics.MetricTypeNameRaw,
				metrics.ConvertMapToTags(map[string]string{
					"podUID":      podUID,
