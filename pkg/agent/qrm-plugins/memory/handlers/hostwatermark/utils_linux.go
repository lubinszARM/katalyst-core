...
func getIntFromFile(path string) (int64, error) {
	data, err := procfsm.ReadFileNoStat(path)
	if err != nil {
		return 0, err
	}
...
func setIntToFile(path string, val int64) error {
	// watermark_scale_factor is a procfs sysctl; use procfs manager to apply it with audit.
	return procfsm.ApplyVMWatermarkScaleFactorAtPath(path, val)
}
