...
func SetHostTCPMem(machineInfo *info.MachineInfo) {
	...
	fmt.Printf("BBLU ok Apply TCPMem: ratio=%d, newLimit=%d, oldLimit=%d", tcpMemRatio, newUpperLimit, tcpMem[2])
...
func SetCg1TCPMem(podUID, containerID string, memLimit, memTCPLimit int64) {
	...
	fmt.Printf("BBLU ok Apply TCPMemLimitInBytes: %v, ratio=%d, memlimit=%d, old value=%d, new value=%d\n", cgroupPath, sockMemConfig.cgroupTCPMemRatio, memLimit, memTCPLimit, newMemTCPLimit)
