...
func SetCg1TCPMem(podUID, containerID string, memLimit, memTCPLimit int) {
	newMemTCPLimit := memLimit / 100 * sockMemConfig.cgroupTCPMemRatio
	...
	fmt.Printf("BBLU Apply TCPMemLimitInBytes: %v, ratio=%d, memlimit=%d, old value=%d, new value=%d\n", cgroupPath, sockMemConfig.cgroupTCPMemRatio, memLimit, memTCPLimit, newMemTCPLimit)
