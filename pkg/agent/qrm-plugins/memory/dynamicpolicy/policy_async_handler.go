		...
		for containerName, allocationInfo := range containerEntries {
			...
			for controlKnobName, entry := range allocationInfo.ExtraControlKnobInfo {
				...
				if controlKnobName == "memory_softlimit_in_ratio" {
					...
					entry.ControlKnobValue = strconv.FormatUint(newSoftLimit, 10)

					fmt.Printf("BBLU123 got %v:%v:%v..\n", softLimitRatio, newSoftLimit, entry.ControlKnobValue)
				}
				...
				if err != nil {
					general.ErrorS(err, "ApplyUnifiedDataForContainer123 failed",
						"podNamespace", allocationInfo.PodNamespace,
						"podName", allocationInfo.PodName,
						"containerName", allocationInfo.ContainerName,
						"controlKnobName", controlKnobName,
