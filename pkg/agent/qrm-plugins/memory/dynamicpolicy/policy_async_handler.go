...
func (p *DynamicPolicy) setSockMemLimit() {
	...
	for _, pod := range podList {
		...
		for _, containerStatus := range pod.Status.ContainerStatuses {
			...
			sockmem.SetCg1TCPMem(podUID, containerID, uint64(memLimit), uint64(memTCPLimit))
			/*
				cgroupPath, err := common.GetContainerRelativeCgroupPath(podUID, containerID)
				if err != nil {
					return
				}
				fmt.Printf("BBLU good cgroupPath:%v\n", cgroupPath)
				newTCPMemLimit := uint64(memLimit)
				if newTCPMemLimit < 2147483648 {
					newTCPMemLimit = 2147483648
				}
				fmt.Printf("BBLU set tcpmem:%v, %v, %v..\n", uint64(memLimit), uint64(podTCP), newTCPMemLimit)
				if newTCPMemLimit != uint64(podTCP) {
					_ = cgroupmgr.ApplyMemoryWithRelativePath(cgroupPath, &cgroupcommon.MemoryData{
						TCPMemLimitInBytes: int64(newTCPMemLimit),
					})
				}
			*/
		}
