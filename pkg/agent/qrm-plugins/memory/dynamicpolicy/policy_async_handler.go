...
func (p *DynamicPolicy) setSockMemLimit() {
	...
	for _, pod := range podList {
		...
		for _, container := range pod.Spec.Containers {
			...
			fmt.Printf("BBLU pod: %s/%s, container: %s", pod.Namespace, pod.Name, containerName)
			podRss, found := helper.GetPodMetric(p.metaServer.MetricsFetcher, p.emitter, pod, coreconsts.MetricMemRssContainer, -1)
			if !found {
				fmt.Printf("BBLU podRSS:%v..\n", podRss)
			}
			//		setExtraControlKnobByConfigForAllocationInfo(allocationInfo, p.extraControlKnobConfigs, pod)
		}
	}
