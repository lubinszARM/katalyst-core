...
func applyIOCostModelWithDefault(ioCostModelConfigs map[DevModel]*common.IOCostModelData, devsIDToModel map[string]DevModel) {
	...
	for devID, _ := range devsIDToModel {
		...
		if (*curModelData) != (*expectedModelData) {
			...
			if err != nil {
				...
				general.Errorf("ApplyIOCostModelWithAbsolutePath for devID: %s successfully",
					devID)
			}
		} else {
			general.Infof("modelData isn't changed, skip ApplyIOCostModelWithAbsolutePath for devID: %s ",
				devID)
		}
	}
}
