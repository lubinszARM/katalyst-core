...
func SetDirtyLimit(conf *coreconfig.Configuration,
	...
	if conf.DirtyRatio != -1 {
		_ = setHostDirtyFile(hostDirtyRatio, uint64(conf.DirtyRatio))
	}

	if conf.DirtyWritebackCentisecs != -1 {
		_ = setHostDirtyFile(hostDirtyWritebackCentisecs, uint64(conf.DirtyWritebackCentisecs))
	}

	if conf.DirtyExpireCentisecs != -1 {
		_ = setHostDirtyFile(hostDirtyExpireCentisecs, uint64(conf.DirtyExpireCentisecs))
	}
}
