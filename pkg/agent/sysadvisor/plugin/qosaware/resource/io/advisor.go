...
import (
	...
	ioadvisorplugin "github.com/kubewharf/katalyst-core/pkg/agent/sysadvisor/plugin/qosaware/resource/memory/plugin"
...
func init() {
	ioadvisorplugin.RegisterInitializer(provisioner.IOProvisioner, provisioner.NewIOProvisioner)
}
...
type ioResourceAdvisor struct {
	...
	startTime time.Time
	plugins   []iodvisorplugin.IOAdvisorPlugin
	mutex     sync.RWMutex
...
func NewIOResourceAdvisor(conf *config.Configuration, extraConf interface{}, metaCache metacache.MetaCache,
	...
	ioAdvisorPluginInitializers := memadvisorplugin.GetRegisteredInitializers()
	for _, memadvisorPluginName := range conf.IOAdvisorPlugins {
		initFunc, ok := ioAdvisorPluginInitializers[memadvisorPluginName]
		if !ok {
			klog.Errorf("failed to find registered initializer %v", memadvisorPluginName)
			continue
		}
		general.InfoS("add new io advisor policy", "policyName", memadvisorPluginName)
