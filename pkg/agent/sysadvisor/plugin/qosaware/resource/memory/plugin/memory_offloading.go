...
func (tmo *transparentMemoryOffloading) Reconcile(status *types.MemoryPressureStatus) error {
	...
	for _, pod := range podList {
		...
		for _, containerStatus := range pod.Status.ContainerStatuses {
			...
			if !enableReclaim {
				if strings.Contains(string(podContainerName), "fdf86e61a6") {
					general.Infof("BBLU podContainerName: %s contains fdf86e61a6, skip disabling TMO", podContainerName)
				} else if containerInfo.IsDedicatedNumaExclusive() {
