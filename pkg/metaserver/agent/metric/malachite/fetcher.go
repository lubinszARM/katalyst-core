...
func (m *MalachiteMetricsFetcher) processCgroupMemoryData(cgroupPath string, cgStats *types.MalachiteCgroupInfo) {
	if cgStats.CgroupType == "V1" {
		...
		m.metricStore.SetCgroupMetric(cgroupPath, consts.MetricMemSocketCgroup, utilmetric.MetricData{Time: &updateTime, Value: float64(mem.KernMemoryTcpLimitInBytes)})
